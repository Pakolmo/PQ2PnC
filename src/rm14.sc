...
(instance rm14 of Room
	...
	(method (init)
		...
		(= gunFireState 3)
		(self setLocales: 153)
		(= local0 (if (== prevRoomNum 33) else (== prevRoomNum 13)))
		(if (== prevRoomNum 33) (Bclr fKeithFollows))
		(= local10
			(if
				(and
					(> global169 0)
				...
				(Btst fStolenCarTowed)
			else
				0
			)
		)
		(= local13 global169)
		(if (or (>= gamePhase 8) local10) (= global169 0))
		(if local0 (HandsOff) else (HandsOn))
		(if (!= global169 0)
			(jailerCar ;(View new:)
				view: 94
				loop: 1
